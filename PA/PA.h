...
class PA
{
	...
	PA(const float& lr_, const float& eps_, bool discreteCase_ = false) : lr(lr_), eps(eps_), discreteCase(discreteCase_)
	{
...
class QPANN : public PA<T>
{
	...
	virtual Mat<T> estimateAction(const Mat<T>& S)	override
	{
		this->mutexPA.lock();
		this->S1 = S;
		//this->A1 = this->maxOutput*clipping(this->net->feedForward( (normalization? normalize( this->S1) : this->S1 ) ));
		this->A1 = clipping(this->net->feedForward( (normalization? normalize( this->S1) : this->S1 ) ));
		//this->A1 = this->maxOutput*clipping(this->net->feedForward( (normalization? normalize( this->S1) : this->S1 ) ));
		
		this->mutexPA.unlock();
		
		if(this->discreteCase)
		{
			int max = idmin( (-1.0f)*this->A1).get(1,1);
			for(int i=1;i<=this->A1.getLine();i++)
			{
				if(i==max)
				{
					this->A1.set( 1.0f, i,1);
				}
				else
				{
					this->A1.set( 0.0f, i,1);
				}
			}
		}
	...
	*/
	...
	virtual Mat<T> eps_greedy(const Mat<T>& S1)	override
	{
		...
		else
		{
			//random :
			Mat<float> ret(this->dimActionSpace,1);
			float amount = 5e-1f;	//50% of exploration...
			
			if(this->discreteCase)
			{
				amount = 1.0f/maxOutput;
				//thus we have a noise that is of the same amplitude as the probability of our actions.
			}
			...
			if(this->discreteCase == false)
			{
				this->previousA1 = this->A1;
				
				ret = ret+this->estimateAction(S1);
				ret = clipping(0.5f*ret+0.5f*this->previousA1);
				//ret = clipping(ret+this->estimateAction(S1));
				this->A1 = ret;
			...
			else
			{
				...
				ret += this->A1;
				int max = idmin( (-1.0f)*ret).get(1,1);
				for(int i=1;i<=ret.getLine();i++)
				{
					if(i==max)
					{
						ret.set( 1.0f, i,1);
