...
void loadDIGITS()
{
	...
	for(int i=0;i<nbrImg/2;i++)
	{
		...
		digits[i] = tresholding( cv2Matp<float>(cv::imread(p)), 110.0f );
		digits[i+nbrImg/2] = extract( rotate(digits[i], theta), offx,offy, offx+(size-1), offy+(size-1) );
	}
}
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	int nbrIterationNeeded = 50000;
	int iteration = nbrIterationNeeded;
	int offx = 2;
	int offy = 2;
	int size = 28;
	int countSuccess = 0;
	...
	*/
	...
	while( iteration)
	{
		...
		Mat<float> inputOriginal( inputDIGITS(labelval) );
		//let us choose the rotation :
		float theta = PI;//((float)(rand()%2))*PI;
		...
		if(labelval < 7)
		{
			...
			*/
			...
			trainer.accumulateGradient( inputO, (1.0f)*(output - target) );
			float momentum = 0.5f;
			trainer(GCSGD,momentum);
			//----------------------------------------
			...
			//----------------------------------------
			//NNTRAINER TEST : rotated
			output = nn.feedForward( inputR);
			...
			trainer.accumulateGradient( inputR, (1.0f)*(output - target) );
			momentum = 0.5f;
			trainer(GCSGD,momentum);
			//----------------------------------------
			...
			*/
			...
			if( iteration % 1000 == 0)
			{
				...
				writeInFile(std::string("./mseTRAINERDIGITS.txt"), mse);
	...
	*/
	...
	nn.save(std::string("neuralnetworksDIGITROTATEDPIDIGITSTRAINER1"));
