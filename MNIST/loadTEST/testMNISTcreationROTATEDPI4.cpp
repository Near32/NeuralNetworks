...
int main(int argc, char* argv[])
{
	...
	unsigned int nbrneurons = nbrinput*2;
	...
	//topo.push_back(nbroutput*4, NTSIGMOID);
	topo.push_back(nbroutput*4, NTRELU);
	
	//topo.push_back(nbroutput, NTSOFTMAX);	//linear output
	topo.push_back(nbroutput, NTSIGMOID);	// output
	//topo.push_back(nbroutput, NTTANH);	//linear output
	...
	*/
	...
	while( iteration)
	{
		...
		if(labelval < 10)
		{
			...
			*/
			...
			trainer.accumulateGradient( inputO, transpose(output - target) );
			float momentum = 0.8f;
			//trainer.update(momentum);
			trainer(GCSGDMomentum,momentum);
			//----------------------------------------
			...
			*/
			...
			trainer.accumulateGradient( inputR, transpose(output - target) );
			//trainer.update(momentum);
			trainer(GCSGDMomentum,momentum);
			//----------------------------------------
			//counting :
	...
	nn.save(std::string("neuralnetworksDIGITROTATEDPI4.NEWTRAINER.ROTATED"));
