...
//#endif	
	...
	/*
	//other strategy : more complex....
	while( i<=nbrFeatures/2)
	{
		//1) let us count the number of association :
		int nbrassoc = 0;
		std::vector<int> assoc;
		for(int j=1;j<=nbrFeatures;j++)
		{
			if(association.get(j,1) == i)
			{
				nbrassoc++;
		...
		//2) if there is one association, okay :
		if( nbrassoc == 1)
		{
			finalAssoc.set( i, i,1);
			finalAssoc.set( assoc[0], i,2);
		}
		else
		{
			//otherwise it means that this point reach the closest for some points, we will take the closer one as association :
			int distmin = tresholddist;
			int idxmin = 0;
			for(int j=0;j<assoc.size();j++)
			{
				if( dist.get( assoc[j], i) < distmin)
				{
					idxmin = assoc[j];
					distmin = dist.get( assoc[j], i);
				}
			}
			...
			{
				finalAssoc.set( i, i,1);
				finalAssoc.set( idxmin, i,2);
			}
			else
			{
				//it means that all of the distance are above the treshold...
				//we must discard that point :
...
class sensorCAM
{
	...
	sensorCAM(float tresh_ = 500.0f, bool erosion_ = false, bool dissect_ = false, bool dilate_ = false) : continuer(true), trustable(false), assoc(Mat<float>(1,1)), tresholddist(tresh_), erosion(erosion), dissect(dissect_), dilate(dilate_)
	{
