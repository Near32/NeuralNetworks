...
class Mat
{
    ...
    public :
		...
		Mat( int line, int column, int depth=1)
		{
			m_line = line;
			m_column = column;
			m_depth = depth;
		...
		Mat( int line, int column, char mode, int depth=1)
		{
			m_line = line;
			m_column = column;
			m_depth = depth;
		...
		Mat( T value, int line, int column, int depth=1)
		{
			m_line = line;
			m_column = column;
			m_depth = depth;
		...
		inline T get(int line, int column,int depth=1) const
		{
		...
		inline T& operator()(const int& line, const int& column, const int& depth=1)
		{
			if(line <= m_line && column <= m_column && line >=1 && column >=1 && depth<=m_depth && depth >=1)
				return mat[(depth-1)*m_line*m_column+(line-1)*m_column+(column-1)];
		...
		inline T operator()(const int& line, const int& column, const int& depth=1)	const
		{
			if(line <= m_line && column <= m_column && line >=1 && column >=1 && depth<=m_depth && depth >=1)
				return mat[(depth-1)*m_line*m_column+(line-1)*m_column+(column-1)];
		...
		inline void set(T value, int line, int column, int depth=1)
		{
			if(line <= m_line && column <= m_column && line >=1 && column >=1 && depth<=m_depth && depth >=1)
...
template<typename T>	/*pas de point virgule en fin de ligne...*/
inline Mat<T> extract(const Mat<T>& m, int ib, int jb, int kb, int ie, int je, int ke);
template<typename T>	/*pas de point virgule en fin de ligne...*/
