...
class Mat
{
    protected :
        ...
        unsigned int m_line;	/*m_line = length(Mat<T>,1) +1*/
        unsigned int m_column;	/*idem...*/                
    ...
    public :
		...
		Mat()
		{
			...
			mat = new T[m_line*m_column*m_depth];
		}
		...
		Mat(const Mat<T>& m)
		{
			...
			for(int i=m_line;i--;)
			{
				...
				{
					for(int k=m_depth;k--;)
						this->set( m.get(i+1,j+1,k+1), i+1,j+1,k+1);
				}
			}
		...
		Mat( unsigned int line, unsigned int column, unsigned int depth=1)
		{
			m_line = line;
			m_column = column;
			m_depth = depth;

			if((m_line<=0)||(m_column<=0)||(m_depth<=0))
				throw;
		...
		Mat( unsigned int line, unsigned int column, char mode, unsigned int depth=1)
		{
			m_line = line;
			m_column = column;
			m_depth = depth;
			...
			srand(time(NULL));
				for(int i=m_line;i--;)
				{
				    for(int j=m_column;j--;)
				    {
				    	for(int k=m_depth;k--;)
				    	{
				        	this->set( (T) (-1)*(rand()%(int)maxi)+(rand()%(int)maxi), i+1,j+1,k+1);
				        }
				    }
				}
			}    
			...
			srand(time(NULL));
				for(int i=m_line;i--;)
				{
				    for(int j=m_column;j--;)
				    {
				    	for(int k=m_depth;k--;)
				    	{
				    		this->set( (T) (rand()%(int)maxi), i+1,j+1,k+1);
				    	}
					}
				}
			}
		...
		Mat( T value, unsigned int line, unsigned int column, unsigned int depth=1)
		{
			m_line = line;
			m_column = column;
			m_depth = depth;
		...
		inline T get(unsigned int line, unsigned int column, unsigned int depth=1) const
		{

			if(line <= m_line && column <= m_column && line >=1 && column >=1 && depth<=m_depth && depth >=1)
				return mat[(depth-1)*m_line*m_column+(line-1)*m_column+(column-1)];

			return 0;
		}


		inline T& operator()(const unsigned int& line, const unsigned int& column, const unsigned int& depth=1)
		{
			if(line <= m_line && column <= m_column && line >=1 && column >=1 && depth<=m_depth && depth >=1)
				return mat[(depth-1)*m_line*m_column+(line-1)*m_column+(column-1)];
		...
		inline T operator()(const unsigned int& line, const unsigned int& column, const unsigned int& depth=1)	const
		{
			if(line <= m_line && column <= m_column && line >=1 && column >=1 && depth<=m_depth && depth >=1)
				return mat[(depth-1)*m_line*m_column+(line-1)*m_column+(column-1)];
		...
		inline void set(T value,const unsigned int& line, const unsigned int& column, const unsigned int& depth=1)
		{
			if(line <= m_line && column <= m_column && line >=1 && column >=1 && depth<=m_depth && depth >=1)
				mat[(depth-1)*m_line*m_column+(line-1)*m_column+(column-1)] = value;
...
template<typename T>	/*pas de point virgule en fin de ligne...*/
Mat<T> Line( const Mat<T>& a, uint ind);
template<typename T>	/*pas de point virgule en fin de ligne...*/
...
template<typename T>	/*pas de point virgule en fin de ligne...*/
inline Mat<T> extract(const Mat<T>& m, uint ib, uint jb, uint kb, uint ie, uint je, uint ke);
template<typename T>	/*pas de point virgule en fin de ligne...*/
...
template<typename T>
Mat<T> conv(const Mat<T>& m, const Mat<T>& k);
template<typename T>	/*pas de point virgule en fin de ligne...*/
...
Mat<T> operator*(const Mat<T>& a, const Mat<T>& b)
{
    ...
    if(a.getColumn() != b.getLine() || a.getDepth() != b.getDepth() )
    {
        cerr << "Impossible d'operer la multiplication : mauvais formats de matrices.\n" << endl;
        cerr << "Format m1 : " << a.getLine() << " x " << a.getColumn() << " x " << a.getDepth() << "\t Fromat m2 : " << b.getLine() << " x " << b.getColumn() << " x " << b.getDepth() << endl;
        throw;
    }
	...
	*/
    ...
    {
		Mat<T> r( a.getLine(), b.getColumn(),a.getDepth());
		...
		for(int l=r.getDepth();l--;)
		{
			for(int i=r.getLine();i--;)
			{
				for(int j=r.getColumn();j--;)
				{
				    ...
				    {
		#ifdef optim
					temp += a.get(i+1,k+1,l+1)*tb.get(j+1,k+1,l+1);
		#else
				        temp += a.get(i+1,k+1,l+1)*tb.get(k+1,j+1,l+1);
		#endif
				    }

				    r.set( temp, i+1, j+1,l+1);
				}
			}
		}
	...
	for(int l=r.getDepth();l--;)
	{
		for(int i=r.getLine();i--;)
		{
		    for(int j=r.getColumn();j--;)
		    {
		        ...
		        for(int k=a->getColumn();k--;)
		        {
			    	temp += a->get(i+1,k+1,l+1)*b->get(j+1,k+1,l+1);
		        }
...
inline Mat<T> operator*(const T& v, const Mat<T>& a)
{
    Mat<T> r(a.getLine(),a.getColumn(),a.getDepth());

    for(int i=r.getLine();i--;)
    {
        for(int j=r.getColumn();j--;)
        {
        	for(int k=r.getDepth();k--;)
        		r.set( (T)(v*a.get(i+1,j+1,k+1)), i+1, j+1, k+1);
...
inline Mat<T> operator*=(Mat<T>& a, const T& v)
{
    for(int i=a.getLine();i--;)
    {
        ...
        {
        	for(int k=a.getDepth();k--;)
        		a.set( v*a.get(i+1,j+1,k+1) , i+1, j+1,k+1);
        }
    }
...
inline Mat<T> operator+(const Mat<T>& a, const Mat<T>& b)
{
    ...
    for(int i=r.getLine();i--;)
    {
        ...
        {
        	for(int k=r.getDepth();k--;)
	            r.set( a.get(i+1,j+1,k+1)+b.get(i+1,j+1,k+1) , i+1, j+1,k+1);
        }
    }
...
inline Mat<T> operator+=(Mat<T>& a, const Mat<T>& b)
{
    ...
    for(int i=a.getLine();i--;)
    {
        ...
        {
        	for(int k=a.getDepth();k--;)
            	a.set( a.get(i+1,j+1,k+1)+b.get(i+1,j+1,k+1) , i+1, j+1,k+1);
        }
    }
...
void regularizeNanM(Mat<T>* m)
{
    ...
    for(int i=m->getLine();i--;)
    {
        for(int j=m->getColumn();j--;)
        {
        	for(int k=m->getDepth();k--;)
        	{
		        if( isnan(m->get(i+1,j+1,k+1) ) )
		        {
		            m->set( 0.0f, i+1,j+1,k+1);
		        }
		    }
        }
    }
...
Mat<T>& Mat<T>::operator=(const Mat<T>& m)
{
    if(this != &m)
    {
        if( !(m.getLine() == this->m_line && m.getColumn() == this->m_column && m.getDepth() == this->m_depth) )
        {
        	this->~Mat();
			this->copy(m);
		}
...
Mat<T> operatorC(const Mat<T>& a, const Mat<T>& b)
{
    if(a.getColumn()==b.getColumn() && a.getDepth() == b.getDepth())
    {
        Mat<T> r(a.getLine()+b.getLine(), a.getColumn(),a.getDepth());
        
        for(int k=1;k<=a.getDepth();k++)
        {
		    for(int i=1;i<=a.getLine();i++)
		    {
		    	for(int j=1;j<=a.getColumn();j++)
		    	{
		    		r.set( a.get(i,j,k), i,j,k);
		    	}
		    }
		    ...
		    for(int i=a.getLine()+1;i<=r.getLine();i++)
		    {
		        ...
		        {
		            r.set( b.get( i-a.getLine(),j, k), i,j,k);
		        }
		    }
		}
...
Mat<T> operatorL(const Mat<T>& a, const Mat<T>& b)
{
    if(a.getLine()==b.getLine() && a.getDepth()==b.getDepth())
    {
        Mat<T> r(a.getLine(), a.getColumn()+b.getColumn(), a.getDepth());
        ...
        for(int k=1;k<=a.getDepth();k++)
        {
		    for(int i=1;i<=a.getLine();i++)
		    {
		        ...
		        {
		            r.set( b.get( i,j-a.getColumn(), k), i,j,k);
		        }
		    }
		}
...
Mat<T> Cola( const Mat<T> a, int ind)
{
    Mat<T> r(a.getLine(), 1,a.getDepth());

    if( ind<= a.getColumn() && ind >=1)
        for(int i=1;i<=a.getLine();i++)
        	for(int k=1;k<=a.getDepth();k++)
        		r.set( a.get(i,ind,k), i, 1,k);
...
inline Mat<T> transpose(const Mat<T>& a)
{
    Mat<T> r(a.getColumn(), a.getLine(),a.getDepth());
	...
	for(int k=a.getDepth();k--;)
	{
		for(int i=a.getColumn();i--;)
		{
		    ...
		    {
		        r.set( a.get(j+1,i+1,k+1), i+1, j+1,k+1);
		    }
		}
	}
...
Mat<T> sigmoidM(const Mat<T>& z)
{
    ...
    for(int i=1;i<=z.getLine();i++)
    {
        ...
        {
        	for(int k=1;k<=z.getDepth();k++)
        	{
            	r.set( sigmoid<T>( (T)(z.get(i,j,k)) ),i,j,k);
            }
        }
    }
...
Mat<T> sigmoidGradM( const Mat<T>& z)
{
    Mat<T> one((T)(1), z.getLine(), z.getColumn(),z.getDepth());
...
Mat<T> log(const Mat<T>& a)
{
    ...
    for(int i=1;i<=r.getLine();i++)
    {
        for(int j=1;j<=r.getColumn();j++)
        {
        	for(int k=1;k<=r.getDepth();k++)
        	{
            	r.set( log<T>(r.get(i,j,k)), i,j,k);
            }
        }
    }
...
Mat<T> expMat( Mat<T>* a)
{
    ...
    for(int i=r.getLine();i--;)
    {
        for(int j=r.getColumn();j--;)
        {
        	for(int k=r.getDepth();k--;)
        	{
            	r.set((T)exp(a->get(i+1,j+1,k+1)), i+1,j+1,k+1);
            }
        }
    }
...
Mat<T> absM(const Mat<T>& a)
{
    ...
    for(int i=r.getLine();i--;)
    {
        for(int j=r.getColumn();j--;)
        {
        	for(int k=r.getDepth();k--;)
        	{
            	r.set((T) fabs_( a.get(i+1,j+1,k+1)), i+1,j+1,k+1);
            }
        }
    }
...
inline Mat<T> extract(const Mat<T>& m, uint ib, uint jb, uint kb, uint ie, uint je, uint ke)
{
    Mat<T> r(ie-ib+1, je-jb+1, ke-kb+1);
    ...
    for(int i=r.getLine();i--;)
    {
        for(int j=r.getColumn();j--;)
        {
        	for(int k=r.getDepth();k--;)
        	{
            	r.set( m(ib+i+1-1,jb+j+1-1,kb+k+1-1), i+1, j+1,k+1);
            }
        }
    }
...
public :
    ...
    LU(Mat<T> A, Mat<T>* L, Mat<T>* U)
    {
        ...
        {
            M[i-1] = new Mat<T>(gaussTrans( Cola(*U, i), i));
            *U = (*M[i-1])*(*U);
    ...
    LU(Mat<T> A)
    {
        ...
        for(int i=1;i<=nbrM-1;i++)
        {
            M[i-1] = new Mat<T>(gaussTrans( Cola(*U_, i), i));
            *U_ = (*M[i-1])*(*U_);
